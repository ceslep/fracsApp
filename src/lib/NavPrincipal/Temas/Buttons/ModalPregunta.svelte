<script>
  import { TEMA } from "./../../../../stores.js";
  import {
    onDestroy,
    createEventDispatcher,
    onMount,
    afterUpdate,
  } from "svelte";
  import "animate.css";
  const dispatch = createEventDispatcher();

  let Modal;
  const modal = (el) => {
    // @ts-ignore
    Modal = new bootstrap.Modal(el);
  };

  afterUpdate(() => {
    console.log("TEMA:", $TEMA);
  });

  onDestroy(() => {
    Modal.dispose;
    Modal = undefined;
  });
  export let show = false;

  $: if (Modal) {
    if (show) Modal.show();
    else Modal.hide();
  }

  const enviarPregunta = async (form) => {
   
    if (!form.target.checkValidity()) {
      form.stopPropagation();
       return
      }

      form.target.classList.add('was-validated')

    console.log(form.target);
    let data = Object.fromEntries(new FormData(form.target));
    console.log(data);
  };
</script>

<article
  use:modal
  class="modal"
  id="staticBackdrop"
  data-bs-backdrop="static"
  data-bs-keyboard="false"
  tabindex="-1"
  aria-labelledby="staticBackdropLabel"
  aria-hidden="true"
>
  <div
    class="modal-dialog 
   modal-dialog-centered
  animate__animated animate__bounceInDown
  "
  >
    <div class="modal-content">
      <header class="modal-header bg-secondary bg-gradient bg-opacity-50">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">
          Preguntar al profesor
        </h1>
        <button
          type="button"
          class="btn-close"
          data-bs-dismiss="modal"
          aria-label="Close"
          on:click={() => {
            dispatch("close");
          }}
        />
      </header>
      <form
      class="needs-validation"  
        on:submit|preventDefault={(frm) => {
          enviarPregunta(frm);
        }}
      >
        <main class="modal-body">
          <div class="row">
            <div class="col-9">
              <div class="form-group">
                <label for="exampleFormControlInput1" class="form-label"
                  >Nombres</label
                >
                <input
                  type="text"
                  class="form-control"
                  name="QuienPregunta"
                  placeholder="Escribe aquí tu nombre"
                  required
                />
                <div class="invalid-feedback">
                  Please choose a username.
                </div>
              </div>
            </div>
            <div class="col-3">
              <img
                src="./preguntaProfesor.png"
                class="img-fluid"
                style="width:120px;"
                alt=""
              />
            </div>
          </div>
          <div class="form-group">
            <label for="textoPregunta" class="form-label">Pregunta</label>
            <textarea
              name="textoPregunta"
              class="form-control"
              id="textoPregunta"
              placeholder="Tema en question: {$TEMA}"
              rows="3"
              required
            />
          </div>
        </main>
        <footer class="modal-footer">
          <button
            type="button"
            class="btn btn-secondary rounded-0"
            data-bs-dismiss="modal"
            on:click={() => {
              dispatch("close");
            }}>Cerrar</button
          >
          <button type="submit" class="btn btn-primary rounded-0">Enviar</button
          >
        </footer>
      </form>
    </div>
  </div>
</article>
